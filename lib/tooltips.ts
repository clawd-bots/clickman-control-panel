export const tooltips: Record<string, string> = {
  ...
  'Meta Pixel': 'Client-side tracking pixel that fires events in the browser when users interact with your site after seeing Meta ads. In Tracking Infrastructure, we prefer the Marketing API `GET /{pixel-id}/stats?aggregation=event` (same event names as Events Manager, including custom events). Optional env `META_PIXEL_ID`; otherwise the first pixel linked to the ad account is used. If `/stats` is unavailable, we fall back to Ads Insights `action_type` rows (often truncated).',
  ...
  'Reddit Pixel': 'Reddit\'s conversion pixel for site and in-app events. Tracking Infrastructure shows ad-account report totals (conversions by ad) from the Reddit Ads API; set optional env `REDDIT_PIXEL_ID` to display your Pixel ID next to the title. Event names in the live breakdown are ad names with conversion counts for the selected date range.',
  ...
  'Account Control Chart': 'Scatter plot of CPA vs. Spend per ad. This tab uses only the Window + Model controls (not the global top-bar date range): Window sets both the report lookback ending today and the Triple Whale attribution_window in SQL. Lifetime uses a 365-day lookback. Ads with spend but no attributed orders or revenue are excluded. Bottom-right = scale; top-right = zombies. Horizontal line = target CPA/NCCPA from Targets (toggle); vertical = 2× that target.',
  'Account Control Campaign Count': 'Number of distinct ad IDs that share this ad name in Triple Whale for the Window (includes inactive / paused ads if they still appear in pixel data). With All campaigns: total across the account. With one campaign selected: count within that campaign only (e.g. same name in multiple ad sets). Not filtered by Meta/TikTok “active campaign” status.',
  ...
  'Demographics Analysis': 'Are you producing for the audience that is actually buying? If women 25-34 drive the bulk of conversions at the lowest CPA, but you keep producing content for other demographics, you\'re misallocating creative resources. Align your production queue with your paying demographic. Platform “All” blends Meta, TikTok, and Reddit (sample) curves. On Spend by Age Over Time, Gender “All” shows two side-by-side stacked columns per period (female ages vs male ages).',
