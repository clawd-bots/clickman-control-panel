...
const pnlRows: PnlRow[] = [
  { label: 'GMV', value: pnlData.gmv.value, color: '#E8C872', isHeader: true, children: pnlData.gmv.items.map(i => ({ label: i.label, value: i.value, pct: i.pct })) },
  ...
  { label: 'CM1', value: pnlData.cm1.value, pct: pnlData.cm1.pct, color: '#E8C872', isHeader: true, children: pnlData.cm1.items.map(i => ({ label: i.label, value: i.value, pct: i.pct })) },
...
export default function PnLPage() {
  ...
  const kpiSparkline = pnlTrend.map(t => t.netRevenue / 1000);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {['Total', 'Quarter', 'Month', 'Week', 'Day'].map(p => (
            <button key={p} onClick={() => setTimePeriod(p)} className={`px-3 py-1.5 rounded-md text-xs font-medium ${timePeriod === p ? 'bg-brand-blue/15 text-brand-blue-light' : 'text-text-tertiary hover:text-text-secondary'}`}>
              {p}
            </button>
          ))}
        </div>
      ...
      <div className="bg-bg-surface border border-border rounded-lg p-5">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={pnlTrend}>
            ...
            <Line type="monotone" dataKey="cm1" name="CM1" stroke="#E8C872" strokeWidth={2} dot={false} />
            ...
            <tbody>
              {pnlRows.map((row) => (
                <>
                  <tr
                    key={row.label}
                    className={`border-b border-border/50 cursor-pointer hover:bg-bg-elevated/50 transition-colors ${row.isHeader ? 'font-semibold' : ''}`}
                    ...
                    <td className="py-3 px-3">
                      <div className="flex items-center gap-2">
                        ...
                        {row.children && row.children.length > 0 ? (
                          expanded[row.label] ? <ChevronDown size={14} className="text-text-tertiary" /> : <ChevronRight size={14} className="text-text-tertiary" />
                        ) : <span className="w-3.5" />}
                        <span className="text-text-primary">{row.label}</span>
                        <InfoTooltip metric={row.label} />
                      </div>
                    </td>
                    <td className="py-3 px-3 text-right text-text-primary">{formatCurrency(row.value)}</td>
                  ...
                  {expanded[row.label] && row.children?.map((child) => (
                    <tr key={`${row.label}-${child.label}`} className="border-b border-border/30 bg-bg-elevated/30">
